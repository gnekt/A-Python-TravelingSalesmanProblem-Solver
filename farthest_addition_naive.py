...
# v 0.1
# example: python3 farthest_addition_naive.py <repeated_version> <apply_local_search> <instance_type> <instance_source_path> <verbose>
...
if __name__ == "__main__":
    print("Farthest addition naive script launched..")
    ...
    if repeated_version:
        tour = repeated_constructive_algorithm(original_instance=instance,constructive_algorithm=farthest_addition_algorithm_naive,verbose=verbose_mode)
        if apply_local_search:
            tour = local_search.local_search(tour=tour,verbose=verbose_mode)
    else:
        if apply_local_search:
            tour = local_search.local_search(original_instance=instance,constructive_algorithm=farthest_addition_algorithm_naive,
                                                  verbose=verbose_mode)
        else:
            tour = farthest_addition_algorithm_naive(original_instance=instance,verbose=verbose_mode)
    tour.plot()
    interface.writer("FarthestAdditionNaive", tour=tour)
