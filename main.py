...
if __name__ == "__main__":
    ...
    print(new_tour.length())
    new_tour.plot()
    tour = repeated_constructive_algorithm(instance, nearest_neighbor_algorithm, verbose=verbose_mode)
    print(tour.length())
    tour.plot()
    local_search = LocalSearch(neighbourood=Neighbourhood.TWO_OPT, exploration=Exploration.FIRST_IMPROVEMENT)
    new_tour = local_search.local_search(tour=tour, verbose=True)
    ...
    print(new_tour.length())
    new_tour.plot()
    tour = repeated_constructive_algorithm(instance, nearest_addition_algorithm, verbose=verbose_mode)
    print(tour.length())
    tour.plot()
    local_search = LocalSearch(neighbourood=Neighbourhood.TWO_OPT, exploration=Exploration.FIRST_IMPROVEMENT)
    new_tour = local_search.local_search(tour=tour, verbose=True)
